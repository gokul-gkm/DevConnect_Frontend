...
export function UsersListPage() {
    ...
    const { users, pagination, isLoading, queryParams, updateParams } = useUsers();
                ...
                >
                    <div className="h-10 w-10 rounded-full bg-gradient-to-br from-slate-800 to-slate-700 flex items-center justify-center shadow-lg ring-1 ring-slate-700/50">
                        {row.original.profilePicture ? 
                            <img src={row.original.profilePicture} className='object-cover rounded-full' alt={row.original.username} /> 
                            : <Users className="h-5 w-5 text-slate-300" />
                        }
                    </div>
                    <div>
                        <div className="font-medium text-slate-100">{row.original.username}</div>
                        <div className="text-xs text-slate-400">ID: {row.original._id.slice(0, 8)}</div>
                    </div>
                </motion.div>
            ),
        },
            ...
            cell: ({ row }) => (
                ...
                >
                    <span className={cn(
                        ...
                        row.original.status === 'active' ? "bg-emerald-400" : "bg-red-400"
                    )} />
