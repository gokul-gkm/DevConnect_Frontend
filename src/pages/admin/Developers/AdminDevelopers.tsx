...
export function AdminDeveloperPage() {
  ...
  const openConfirmDialog = (dev: any) => {
    setConfirmDialog({
      open: true,
      developerId: dev.userId._id,
      username: dev.userId.username,
      action: dev.userId.status === "active" ? "block" : "unblock",
    });
  };

  const handleConfirm = () => {
    toggleStatusMutation.mutate(confirmDialog.developerId, {
          ...
          >
            ...
            <div>
              ...
              <div className="text-xs text-slate-400">
                ID: {row.original.userId._id.slice(0, 8)}
              </div>
            </div>
          </motion.div>
        ),
      ...
      <ConfirmActionDialog
        ...
        description={
          confirmDialog.action === "block"
            ? `Are you sure you want to block developer "${confirmDialog.username}"?`
            : `Are you sure you want to unblock developer "${confirmDialog.username}"?`
        }
        icon={ShieldAlert}
        iconColor={
          confirmDialog.action === "block" ? "text-red-500" : "text-green-500"
        }
        pulseColor={
          confirmDialog.action === "block" ? "bg-red-500/20" : "bg-green-500/20"
        }
        confirmText={
          confirmDialog.action === "block"
            ? "Block Developer"
