...
export default function SessionDetails() {
  ...
  const { sessionActive } = useCallContext();
  ...
  return (
    <div className="relative w-full rounded-2xl bg-black/40 backdrop-blur-xl">
      <PaymentSection
    ...
    if (
  (session.status === 'approved' || session.status === 'scheduled' || session.status === 'active') &&
        ...
        >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-8">
            <div className="lg:col-span-2 space-y-4 sm:space-y-8">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {session.topics.map((topic: string) => (
                      <Badge
                        key={topic}
                        variant="outline"
                        className="px-3 py-1.5 sm:px-4 sm:py-2 rounded-xl sm:rounded-xl bg-white/5 text-white border-white/10 hover:bg-white/10 text-sm"
                      >
                        {topic}
                      </Badge>
                    ))}
              ...
              <div className="flex gap-3 sm:gap-4 justify-end w-full">
                    {canCancelSession && (
                  <Button
                    variant="destructive"
                    className="h-12 gap-2 rounded-xl bg-rose-950/50 hover:bg-rose-900/50 text-rose-400 border border-rose-500/30 shadow-lg shadow-rose-900/5 hover:shadow-rose-900/10 transition-all duration-300"
